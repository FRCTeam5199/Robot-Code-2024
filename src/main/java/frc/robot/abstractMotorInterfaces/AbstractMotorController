...
public abstract class AbstractMotorController {
    ...
    protected boolean isTemperatureAcceptable() {
        if (MainConstants.ENABLE_OVERHEAT_DETECTION) {
            if (getMotorTemperature() >= MainConstants.OVERHEAT_THRESHOLD) {
                if (!isOverheated) {
                    UserInterface.smartDashboardPutBoolean("OVERHEAT " + getID(), false);
                    isOverheated = true;
                }
            } //wait 5 degrees to unoverheat
            else if (isOverheated && getMotorTemperature() < MainConstants.OVERHEAT_THRESHOLD - 5) {
                isOverheated = false;
